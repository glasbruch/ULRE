...
def calculate_auroc(examples, labels):
    """Calculate Area Under Receiver Operating Characteristic curve."""
    return sk.roc_auc_score(labels, examples)

def calculate_aupr(examples, labels):
    """Calculate Area Under Precision-Recall curve."""
    return sk.average_precision_score(labels, examples)

def calculate_fpr(examples, labels, recall_level):
    """Calculate False Positive Rate at given recall level."""
    return fpr_and_fdr_at_recall(labels, examples, recall_level)

def get_measures_parallel(_pos, _neg, recall_level=recall_level_default, n_jobs=None):
    # If n_jobs is not specified, use all available cores minus 1 (to avoid system slowdown)
    if n_jobs is None:
    ...
    # Prepare functions with fixed arguments for multiprocessing
    tasks = [
        (calculate_auroc, (examples, labels, )),
        (calculate_aupr, (examples, labels, )),
        (calculate_fpr, (examples, labels, recall_level))
    ]
...
def get_and_print_results(out_score, in_score, num_to_avg=1):
    ...
    measures = get_measures_parallel(out_score, in_score)
...
def benchmark_metrics(sizes, n_runs=5):
    """
    Benchmark the performance of sequential vs parallel metrics calculation.
    
    Parameters
    ----------
    sizes : list of int
        List of dataset sizes to benchmark.
    n_runs : int, optional (default=5)
        Number of runs to average over for each size.
    ...
    for size in sizes:
        ...
        pos_examples = np.random.normal(0.7, 0.3, size)
        neg_examples = np.random.normal(0.4, 0.3, size)
        
        # Sequential benchmark
        seq_times = []
        for _ in range(n_runs):
            start_time = time.time()
            get_measures(pos_examples, neg_examples)
        ...
        for _ in range(n_runs):
            start_time = time.time()
            get_measures_parallel(pos_examples, neg_examples)
        ...
        # Store results
        results['sequential_times'].append(avg_seq_time)
        results['parallel_times'].append(avg_par_time)
        results['speedups'].append(speedup)
        
        print(f"  Sequential: {avg_seq_time:.4f}s")
        print(f"  Parallel:   {avg_par_time:.4f}s")
...
def plot_benchmark_results(results):
    ...
    ax1.plot(results['sizes'], results['sequential_times'], 'o-', label='Sequential')
    ax1.plot(results['sizes'], results['parallel_times'], 's-', label='Parallel')
    ...
    # Plot speedup
    ax2.plot(results['sizes'], results['speedups'], 'o-', color='green')
...
if __name__ == "__main__":
    ...
    # Run benchmark
    print(f"Running benchmark with {cpu_count()} available CPU cores")
    results = benchmark_metrics(sizes)
    ...
    # Print summary
    print("\nBenchmark Summary:")
    print(f"Maximum speedup: {max(results['speedups']):.2f}x with {sizes[results['speedups'].index(max(results['speedups']))]} samples")
